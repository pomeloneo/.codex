...
Prefer Codex's built-in image understanding when the conversion is being run inside a Codex task: convert the document with MarkItDown, extract or screenshot the relevant images/slides, have Codex inspect them, then write or refine the Markdown description directly. For larger batches, split the extracted images/slides into groups and process them in parallel Codex subagents when that capability is available.

Use an external OpenAI-compatible API only when the conversion must run as a standalone script outside a Codex-managed task, for example in CI or a plain scheduled job:
...
# For OpenAI directly, omit base_url and use OPENAI_API_KEY.
# For OpenRouter or another proxy, set base_url accordingly.
client = OpenAI(
...
md = MarkItDown(
    llm_client=client,
    llm_model="gpt-4o-mini",  # example; change to any vision-capable model you have access to
...
# Use an OpenAI-compatible API only when this batch job runs outside a Codex-managed task
client = OpenAI(
    api_key="your-api-key",
    # base_url="https://openrouter.ai/api/v1"
)
...
- **OpenAI API**: https://platform.openai.com/docs (optional for batch AI-enhanced conversions)
- **OpenRouter**: https://openrouter.ai (optional OpenAI-compatible routing for batch AI-enhanced conversions)
- **MCP Server**: markitdown-mcp (optional MCP integration; configure through Codex MCP settings if used)
