...
<required_reading>
@$HOME/.codex/gsd-core/references/domain-probes.md
@$HOME/.codex/gsd-core/references/gate-prompts.md
@$HOME/.codex/gsd-core/references/universal-anti-patterns.md
</required_reading>
...
if [ -f "$HOME/.codex/gsd-core/USER-PROFILE.md" ]; then
  ADVISOR_MODE=true
else
  ADVISOR_MODE=false
fi
...
Read `@$HOME/.codex/gsd-core/references/scout-codebase.md` — it contains the phase-type→map selection table, single-read rule, no-maps fallback, and `<codebase_context>` output schema. Then execute:
